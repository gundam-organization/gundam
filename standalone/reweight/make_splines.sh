...
while getopts ":hTLNDd:i:n:b:o:" opt; do
    case ${opt} in
        h )
            echo "USAGE: "
            ...
            echo "-D: Create dummy spline inputs"
            exit 0
            ;;
        i )
...
do
    ...
    ./xsllh_response_functions.exe -i ${HIGHLAND_INPUT} -o ${FILE_WEIGHTS} -d ${FILE_DIALS} -r ${dial} -n ${NUM_STEPS} ${READ_LIMITS} ${TRUTH_TREE} ${DUMMY_SPLINES}
