...
void systPlots_dpt(string microtreeIn, string rwpath, string dialName, double nom, double err, int topo, int dataType=0) {
  ...
  const int nRbins = 8;
  const double Rbins[nRbins+1] = { 0.0, 0.08, 0.12, 0.155, 0.2, 0.26, 0.36, 0.51, 1.1};

  // set up true binning
  const int nTbins = 8;
  const double Tbins[nTbins+1] = { 0.0, 0.08, 0.12, 0.155, 0.2, 0.26, 0.36, 0.51, 1.1};
  // const int nTbins = 1;
  // const double Tbins[nTbins+1] = { 0.0, 100.0};

  //Extra PS bins, not binned in var of interest
  const int nextrabins=0;
  ...
  */
  ...
  TH1 *htrue[nWeights][nTotalBins][nTopologies];

  TH1D* hrecowbins[nWeights];
  for(const int w = 0; w < nWeights; w++){
    hrecowbins[w] = new TH1D(Form("%s_weight_%d",dialName.c_str(),w),Form("%s_weight_%d",dialName.c_str(),w), nRbins, Rbins);
    if(w<3) hrecowbins[w]->SetLineStyle(2);
    if(w>3) hrecowbins[w]->SetLineStyle(3);
    hrecowbins[w]->SetLineColor(w+2);
    if(w==3) { // nominal
      hrecowbins[w]->SetLineStyle(1);
      hrecowbins[w]->SetLineColor(1);
      hrecowbins[w]->SetTitle("Nominal");
    }
    hrecowbins[w]->SetAxisRange(0.,35000.,"Y");
    //hrecowbins[w]->GetYaxis()->SetLimits(0,35000);
    hrecowbins[w]->GetYaxis()->SetTitle("Events/bin width");
    hrecowbins[w]->GetXaxis()->SetTitle("\\delta p_T (GeV)");
  }
  ...
  int t = topo;
  if(t==0 || t==4) continue; // Ignore branches with no proton

  cout << "Current bins (global dpt): " << globalCount << endl << endl;
  for(const int w = 0; w < nWeights; w++){
    ...
    cut = Form( "((Sum$(accum_level[xstool_throw][%d]) > %d))", t, accumToCut[t]);
    ...
    //if(t==5 || t==6) bcut = Form(   "( (Sum$(trueDpT[xstool_throw]) > %f) && (Sum$(trueDpT[xstool_throw])) < %f )  ", Tbins[bt],Tbins[bt+1]);
    //else bcut = Form( "((Sum$(trueDpT[xstool_throw]) > %f) && (Sum$(trueDpT[xstool_throw])) < %f) && ((Sum$(truep_truemom[xstool_throw]) > %f) && (Sum$(truep_truemom[xstool_throw])) < %f) && ((Sum$(truep_truecostheta[xstool_throw]) > %f) && (Sum$(truep_truecostheta[xstool_throw])) < %f) && ((Sum$(truemu_mom[xstool_throw]) > %f) && (Sum$(truemu_mom[xstool_throw])) < %f) && ((Sum$(truemu_costheta[xstool_throw]) > %f) && ((Sum$(truemu_costheta[xstool_throw])) < %f)) ", Tbins[bt],Tbins[bt+1],pmombins[0],pmombins[1],pthetabins[0],pthetabins[1],mumombins[0],mumombins[1],muthetabins[0],muthetabins[1]);
    ...
    htrue[w][globalCount][t] = inpRwFly->GetTruth();
  ...
  //for(int br = 0; br<globalCount; br++){//reco kinematics bin
    for(int bt = 0; bt < globalCount; bt++){//true kinematics bin
      ...
      output->cd(dir);
      char nameHisto[256];
      sprintf(nameHisto,"trueBin_%d_topology_%d",bt,t);
      ...
      for(int w=0;w<nWeights;w++){
        ...
        hreco[w][bt][t]->Write(Form("recoHisto_weight_%d_trueBin_%d_topology_%d",w,bt,t));
        //htrue[w][bt][t]->Write(Form("trueHisto_weight_%d_trueBin_%d_topology_%d",w,bt,t));
        for( int bin=0; bin<nRbins+1; bin++){
          hrecowbins[w]->SetBinContent(bin, (hreco[w][bt][t]->GetBinContent(bin+1)));
        }
        hrecowbins[w]->Write(Form("binnedHisto_weight_%d_trueBin_%d_topology_%d",w,bt,t));
        hrecowbins[w]->Scale(1, "width");

        hrecowbins[w]->Write(Form("binnedScaledHisto_weight_%d_trueBin_%d_topology_%d",w,bt,t));
        ...
        ReWeight[bt][t]->GetXaxis()->SetTitle(Form("%s_scale",dialName.c_str()));
      }     
      for(int w=0;w<nWeights;w++){
        hrecowbins[w]->SetAxisRange(0.,37000.,"Y");
        if(w==0){
          hrecowbins[w]->Draw();
        }
        else{
          hrecowbins[w]->Draw("same");
        }
      }
      canv->BuildLegend(0.4,0.5,0.75,0.88);
      canv->Write("error_comp");
      canv->SaveAs(Form("%s_topo_%d_hist.png",dialName.c_str(), topo));
      canv->SaveAs(Form("%s_topo_%d_histo.png",dialName.c_str(), topo));

      TCanvas* canv2 = new TCanvas("canv2","canv2",900,700);
      ReWeight[bt][t]->Write();
      ReWeight[bt][t]->Draw("APL");
      canv2->SaveAs(Form("%s_topo_%d_graph.png",dialName.c_str(), topo));
