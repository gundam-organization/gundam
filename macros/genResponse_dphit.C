...
void genResponse_dphit(string microtreeIn, string rwpath, string dialName, double nom, double err, int topo, int reac, int dataType=0) {
  ...
  */
  ...
  for(const int bt = 0; bt < nTbins; bt++){
    ...
    cout << "Current bins (global dphit): " << globalCount << " " << bt << endl << endl;
    for(const int w = 0; w < nWeights; w++){
      ...
      if(t==5 || t==6) bcut = Form(  " ( (Sum$(trueDphiT[xstool_throw]) > %f) && (Sum$(trueDphiT[xstool_throw])) < %f )  ", Tbins[bt],Tbins[bt+1]);
      else bcut = Form( "((Sum$(trueDphiT[xstool_throw]) > %f) && (Sum$(trueDphiT[xstool_throw])) < %f) && ((Sum$(truep_truemom[xstool_throw]) > %f) && (Sum$(truep_truemom[xstool_throw])) < %f) && ((Sum$(truep_truecostheta[xstool_throw]) > %f) && (Sum$(truep_truecostheta[xstool_throw])) < %f) && ((Sum$(truemu_mom[xstool_throw]) > %f) && (Sum$(truemu_mom[xstool_throw])) < %f) && ((Sum$(truemu_costheta[xstool_throw]) > %f) && ((Sum$(truemu_costheta[xstool_throw])) < %f)) ", Tbins[bt],Tbins[bt+1],pmombins[0],pmombins[1],pthetabins[0],pthetabins[1],mumombins[0],mumombins[1],muthetabins[0],muthetabins[1]);
