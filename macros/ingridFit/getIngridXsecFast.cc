...
void calcXsecWithErrors(TString mcFilename, TString fitResultFilename, TString fakeDataFilename, TString outputFileName,
                        double potRatio)
{
    ...
    TH1D* totalSpectMC_allSam
        = new TH1D("totalSpectMC_allSam", "totalSpectMC_allSam", nbinsAutoSample, binsAutoSample->GetArray());
    TH1D* totalSpectFit_allSam
        = new TH1D("totalSpectFit_allSam", "totalSpectFit_allSam", nbinsAutoSample, binsAutoSample->GetArray());
    TH1D* totalSpectFD_allSam
        = new TH1D("totalSpectFD_allSam", "totalSpectFD_allSam", nbinsAutoSample, binsAutoSample->GetArray());

    TH1D* totalSpectMC_sam0 = (TH1D*)fitResultFile->Get("evhist_sam0_iter0_mc");
    // TH1D* totalSpectFit_sam0 = (TH1D*) fitResultFile->Get("evhist_sam0_iter"+finalIter+"_pred");  //Need to insert
    // correct iter number
    TH1D* totalSpectFit_sam0
        = (TH1D*)fitResultFile->Get("evhist_sam0_finaliter_pred"); // Need to insert correct iter number
    ...
    if (!totalSpectMC_sam1)
        totalSpectMC_sam1 = new TH1D("NullSam1", "NullSam1", nbinsAutoSample, binsAutoSample->GetArray());
    if (!totalSpectFit_sam1)
        totalSpectFit_sam1 = new TH1D("NullSam1", "NullSam1", nbinsAutoSample, binsAutoSample->GetArray());
    if (!totalSpectFD_sam1)
        totalSpectFD_sam1 = new TH1D("NullSam1", "NullSam1", nbinsAutoSample, binsAutoSample->GetArray());
    ...
    if (totalSpectMC_sam2)
        totalSpectMC_allSam->Add(totalSpectMC_sam2);
    if (totalSpectFit_sam2)
        totalSpectFit_allSam->Add(totalSpectFit_sam2);
    if (totalSpectFD_sam2)
        totalSpectFD_allSam->Add(totalSpectFD_sam2);
    if (!totalSpectMC_sam2)
        totalSpectMC_sam2 = new TH1D("NullSam2", "NullSam2", nbinsAutoSample, binsAutoSample->GetArray());
    if (!totalSpectFit_sam2)
        totalSpectFit_sam2 = new TH1D("NullSam2", "NullSam2", nbinsAutoSample, binsAutoSample->GetArray());
    if (!totalSpectFD_sam2)
        totalSpectFD_sam2 = new TH1D("NullSam2", "NullSam2", nbinsAutoSample, binsAutoSample->GetArray());
    ...
    if (totalSpectMC_sam3)
        totalSpectMC_allSam->Add(totalSpectMC_sam3);
    if (totalSpectFit_sam3)
        totalSpectFit_allSam->Add(totalSpectFit_sam3);
    if (totalSpectFD_sam3)
        totalSpectFD_allSam->Add(totalSpectFD_sam3);
    if (!totalSpectMC_sam3)
        totalSpectMC_sam3 = new TH1D("NullSam3", "NullSam3", nbinsAutoSample, binsAutoSample->GetArray());
    if (!totalSpectFit_sam3)
        totalSpectFit_sam3 = new TH1D("NullSam3", "NullSam3", nbinsAutoSample, binsAutoSample->GetArray());
    if (!totalSpectFD_sam3)
        totalSpectFD_sam3 = new TH1D("NullSam3", "NullSam3", nbinsAutoSample, binsAutoSample->GetArray());
    ...
    if (totalSpectMC_sam4)
        totalSpectMC_allSam->Add(totalSpectMC_sam4);
    if (totalSpectFit_sam4)
        totalSpectFit_allSam->Add(totalSpectFit_sam4);
    if (totalSpectFD_sam4)
        totalSpectFD_allSam->Add(totalSpectFD_sam4);
    if (!totalSpectMC_sam4)
        totalSpectMC_sam4 = new TH1D("NullSam4", "NullSam4", nbinsAutoSample, binsAutoSample->GetArray());
    if (!totalSpectFit_sam4)
        totalSpectFit_sam4 = new TH1D("NullSam4", "NullSam4", nbinsAutoSample, binsAutoSample->GetArray());
    if (!totalSpectFD_sam4)
        totalSpectFD_sam4 = new TH1D("NullSam4", "NullSam4", nbinsAutoSample, binsAutoSample->GetArray());
    ...
    if (totalSpectMC_sam5)
        totalSpectMC_allSam->Add(totalSpectMC_sam5);
    if (totalSpectFit_sam5)
        totalSpectFit_allSam->Add(totalSpectFit_sam5);
    if (totalSpectFD_sam5)
        totalSpectFD_allSam->Add(totalSpectFD_sam5);
    if (!totalSpectMC_sam5)
        totalSpectMC_sam5 = new TH1D("NullSam5", "NullSam5", nbinsAutoSample, binsAutoSample->GetArray());
    if (!totalSpectFit_sam5)
        totalSpectFit_sam5 = new TH1D("NullSam5", "NullSam5", nbinsAutoSample, binsAutoSample->GetArray());
    if (!totalSpectFD_sam5)
        totalSpectFD_sam5 = new TH1D("NullSam5", "NullSam5", nbinsAutoSample, binsAutoSample->GetArray());
    ...
    if (totalSpectMC_sam6)
        totalSpectMC_allSam->Add(totalSpectMC_sam6);
    if (totalSpectFit_sam6)
        totalSpectFit_allSam->Add(totalSpectFit_sam6);
    if (totalSpectFD_sam6)
        totalSpectFD_allSam->Add(totalSpectFD_sam6);
    if (!totalSpectMC_sam6)
        totalSpectMC_sam6 = new TH1D("NullSam6", "NullSam6", nbinsAutoSample, binsAutoSample->GetArray());
    if (!totalSpectFit_sam6)
        totalSpectFit_sam6 = new TH1D("NullSam6", "NullSam6", nbinsAutoSample, binsAutoSample->GetArray());
    if (!totalSpectFD_sam6)
        totalSpectFD_sam6 = new TH1D("NullSam6", "NullSam6", nbinsAutoSample, binsAutoSample->GetArray());
    ...
    if (totalSpectMC_sam7)
        totalSpectMC_allSam->Add(totalSpectMC_sam7);
    if (totalSpectFit_sam7)
        totalSpectFit_allSam->Add(totalSpectFit_sam7);
    if (totalSpectFD_sam7)
        totalSpectFD_allSam->Add(totalSpectFD_sam7);
    if (!totalSpectMC_sam7)
        totalSpectMC_sam7 = new TH1D("NullSam7", "NullSam7", nbinsAutoSample, binsAutoSample->GetArray());
    if (!totalSpectFit_sam7)
        totalSpectFit_sam7 = new TH1D("NullSam7", "NullSam7", nbinsAutoSample, binsAutoSample->GetArray());
    if (!totalSpectFD_sam7)
        totalSpectFD_sam7 = new TH1D("NullSam7", "NullSam7", nbinsAutoSample, binsAutoSample->GetArray());
    ...
    if (totalSpectMC_sam8)
        totalSpectMC_allSam->Add(totalSpectMC_sam8);
    if (totalSpectFit_sam8)
        totalSpectFit_allSam->Add(totalSpectFit_sam8);
    if (totalSpectFD_sam8)
        totalSpectFD_allSam->Add(totalSpectFD_sam8);
    if (!totalSpectMC_sam8)
        totalSpectMC_sam8 = new TH1D("NullSam8", "NullSam8", nbinsAutoSample, binsAutoSample->GetArray());
    if (!totalSpectFit_sam8)
        totalSpectFit_sam8 = new TH1D("NullSam8", "NullSam8", nbinsAutoSample, binsAutoSample->GetArray());
    if (!totalSpectFD_sam8)
        totalSpectFD_sam8 = new TH1D("NullSam8", "NullSam8", nbinsAutoSample, binsAutoSample->GetArray());
    ...
    TH1D* mcSpect_ingrid = new TH1D("mcSpect_ingrid", "mcSpect_ingrid", nbinsAuto, binsAuto->GetArray());
    TH1D* fakeDataTrueSpect_ingrid
        = new TH1D("fakeDataTrueSpect_ingrid", "fakeDataTrueSpect_ingrid", nbinsAuto, binsAuto->GetArray());
    TH1D* mcSpect_ingrid_T = new TH1D("mcSpect_T", "mcSpect_T", nbinsAuto, binsAuto->GetArray());
    TH1D* fakeDataTrueSpect_ingrid_T
        = new TH1D("fakeDataTrueSpect_T", "fakeDataTrueSpect_T", nbinsAuto, binsAuto->GetArray());

    selEvtsTree->Draw("(D1True)>>mcSpect",
                      "weight*(( mectopology<3 ) && D2True==0 && ((cutBranch!=0) && (cutBranch!=4)))");
    selEvtsFakeDataTree->Draw("(D1True)>>fakeDataTrueSpect",
                              "weight*(( mectopology<3 ) && D2True==0 && ((cutBranch!=0) && (cutBranch!=4)))");

    selEvtsTree->Draw("(D1True)>>mcSpect_ingrid",
                      "weight*(( mectopology<3 ) && D2True==1 && ((cutBranch!=0) && (cutBranch!=4)))");
    selEvtsFakeDataTree->Draw("(D1True)>>fakeDataTrueSpect_ingrid",
                              "weight*(( mectopology<3 ) && D2True==1 && ((cutBranch!=0) && (cutBranch!=4)))");
    ...
    for (int i = 1; i <= nbinsAuto; i++) {
        ...
        cout << "Error for bin " << i << " is " << ((fitSpect->GetBinContent(i)) * (fitParamErrors->GetBinContent(i)))
             << endl;
