...
//                5 - Weight events in mu TPC p FGD topology by 1.5 (500<p_p<700, cos(theta_p)<0.7 )
//                6 - Weight events in mu TPC p FGD topology by 0.5 
...
int treeConvert_paper(TString inFileName, TString inTreeName,  TString inTreeName_T, TString outFileName,
                ...
                TString D2NameRec, TString D2NameTrue, TString D2NameTrue_T, bool isRealData=false, bool psdim=true,  bool useAltPp=false)
{
  ...
  const int accumToCut[nbranches] =   {7,8,9,8,7,5,4,7,8,7}; // Get all branches
  //const int accumToCut[nbranches] =   {8,8,9,8,8,5,4,7,9,8}; // Just for weight matching with postfit, in general do not use!
  ...
  Float_t        hmpMomRec;
  Float_t        hmpMomTrue;
  Float_t        hmpDirRec[1500][3];
  Float_t        hmpDirTrue[3];
  ...
  Float_t        infpMomZRec;
  Float_t        infpMom;
  Float_t        infpCosTheta;

  Float_t        infpMomDif;
  Float_t        infpAngleDif;
  Float_t        inf3MomDif;
  Float_t        infxMomDif;
  Float_t        infyMomDif;
  Float_t        infzMomDif;
  ...
  intree->SetBranchAddress("selp_truemom" ,&pMomTrueAlt);
  intree->SetBranchAddress("selp_trueztheta" ,&pThetaTrueAlt);
  intree->SetBranchAddress("selp_dir" ,&pMomDir);

  intree->SetBranchAddress("selmu_mom", &muMomRec);
  intree->SetBranchAddress("selmu_mom_range_oarecon", &muMomRecRange);
  intree->SetBranchAddress("selmu_costheta", &muCosThetaRec);
  intree->SetBranchAddress("truelepton_mom", &muMomTrue);
  intree->SetBranchAddress("truelepton_costheta", &muCosThetaTrue);

  intree->SetBranchAddress("selHMP_mom", &hmpMomRec);
  intree->SetBranchAddress("selHMP_truemom", &hmpMomTrue);
  intree->SetBranchAddress("selHMP_dir", &hmpDirRec);
  intree->SetBranchAddress("selHMP_truedir", &hmpDirTrue);

  intree->SetBranchAddress("selmu_infp_threemomx", &infpMomXRec);
  intree->SetBranchAddress("selmu_infp_threemomy", &infpMomYRec);
  intree->SetBranchAddress("selmu_infp_threemomz", &infpMomZRec);
  intree->SetBranchAddress("selmu_infp_mom", &infpMom);
  intree->SetBranchAddress("selmu_infp_costheta", &infpCosTheta);
  ...
  intree->SetBranchAddress("weight", &weightIn);
  ...
  outtree->Branch("hmpMomRec", &hmpMomRec, ("hmpMomRec/F"));
  outtree->Branch("hmpMomTrue", &hmpMomTrue, ("hmpMomTrue/F"));
  outtree->Branch("hmpCosThetaRec", &hmpCosThetaRec, ("hmpCosThetaRec/F"));
  outtree->Branch("hmpCosThetaTrue", &hmpCosThetaTrue, ("hmpCosThetaTrue/F"));
  outtree->Branch("infpMomDif", &infpMomDif, ("infpMomDif/F"));
  outtree->Branch("infAngleDif", &infpAngleDif, ("infAngleDif/F"));
  outtree->Branch("inf3MomDif", &inf3MomDif, ("inf3MomDif/F"));
  ...
  intree_T->SetBranchAddress("truehm_proton_truemom" ,&pMomTrue_T);
  intree_T->SetBranchAddress("truehm_proton_truecth" ,&pCosThetaTrue_T);
  ...
  intree_T->SetBranchAddress("weight", &weightIn_T);
  ...
  for (Long64_t jentry=EvtStart; jentry<nentries;jentry++) {
    ...
    weight=weightIn[0]*weightIn[1];
    //muCosThetaRec  = TMath::Cos(muThetaRec);
    //muCosThetaTrue = TMath::Cos(muThetaTrue);

    // truep is (or at least used to be) the first final state proton, which is very arbitary
    // using selp_truemom ensures that you're always getting the truth info for the proton that you atually see
    // However, for background events it's possible that this isn't actually a proton, but this doesn't matter since
    // we don't really put bkg events into the OOPS bin 
    if(useAltPp){
    ...
    for(int i=0; i<nbranches; i++){
      if(accum_level[0][i]>accumToCut[i]){
        ...
        infpMomDif=infpMom-pMomRec;
        infpAngleDif=acos(infpCosTheta)-acos(pCosThetaRec);
        infxMomDif=infpMomXRec-(pMomRec*pMomDir[0]);
        infyMomDif=infpMomXRec-(pMomRec*pMomDir[1]);
        infzMomDif=infpMomXRec-(pMomRec*pMomDir[2]);
        inf3MomDif=sqrt(infxMomDif*infxMomDif + infyMomDif*infyMomDif + infzMomDif*infzMomDif);

        if(isRealData && weight==0) weight=1.0;

        hmpCosThetaRec=hmpDirRec[0][2];
        hmpCosThetaTrue=hmpDirTrue[2];
        ...
        if(specialRWmode==5 && pMomTrue>500 && pMomTrue<700 && muCosThetaTrue<0.7) weight=1.5;
        if(specialRWmode==6 && pMomTrue>500 && pMomTrue<700 && muCosThetaTrue<0.7) weight=0.5;
  ...
  for (Long64_t jentry=0; jentry<nentries_T;jentry++) {
    //weight_T=1;
    weight_T=weightIn_T[0];
    nb_T = intree_T->GetEntry(jentry); nbytes_T += nb_T;
    if(specialRWmode==1 && reaction_T==9) weight_T=0;
    if(specialRWmode==2 && reaction_T==9) weight_T=2*weight_T;
    if(specialRWmode==3 && reaction_T==9) weight_T=3*weight_T;
    ...
    if(specialRWmode==5 && pMomTrue_T>500 && pMomTrue_T<700 && muCosThetaTrue_T<0.7) weight_T=weight_T*1.5;
    if(specialRWmode==6 && pMomTrue_T>500 && pMomTrue_T<700 && muCosThetaTrue_T<0.7) weight_T=weight_T*0.5;
    outtree_T->Fill();
  }
