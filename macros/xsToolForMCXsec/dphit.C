...
void cctransdphi() {
  ...
  *********************************************************/
  ...
  inp->SetCut("Sum$(accum_level[xstool_throw]) > 7");
  ...
  const int nbins = 8;
  const double bins[nbins+1] = { 0.0, 0.067, 0.14, 0.225, 0.34, 0.52, 0.85, 1.5, 3.14159};
  std::cout << "Adding binning ... " << std::endl;
  binningTru->addDimension(string("trueDphiT"),nbins,bins);
  binningRec->addDimension(string("Sum$(recDphiT[xstool_throw])"),nbins,bins);
...
//engine->SetTargetMode(xsTargetSection::FGD1, xsTargetNucleons::BOTH);  // protons and neutrons in FGD1 are targets
  ...
  cout << "************************************************************************" << endl;
  //ut << "Total xsec = ( " << totalXS << " +- " << staterr << "(stat) +- " << systerr << "(syst) ) x 10^-38 cm^2 / Nucleon" << endl;
  ...
  *********************************************************/
   ...
   TFile *outputf = new TFile("dphit_MCTruth.root","RECREATE");
