...
void cctransdphi() {
  ...
  *********************************************************/
  ...
  const int nbins = 8;
  const double bins[nbins+1] =  { 0.0, 0.47, 1.02, 1.54, 1.98, 2.34, 2.64, 2.89, 3.14159};
  std::cout << "Adding binning ... " << std::endl;
  binningTru->addDimension(string("trueDalphaT"),nbins,bins);
  binningRec->addDimension(string("Sum$(recDalphaT[xstool_throw])"),nbins,bins);
  ...
  *********************************************************/
  ...
  TFile *outputf = new TFile("dat_MCTruth.root","RECREATE");
