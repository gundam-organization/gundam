...
void cctransdphi() {
  ...
  *********************************************************/
  ...
  //NEUT MC NEUT DATA
    //inpRun->SetupFromHighland("/data/t2k/dolan/xsToolBasedMEC/CC0PiAnl/microtrees/mar16/job_NeutAirAllSystV5_out/allMerged.root","/data/t2k/dolan/xsToolBasedMEC/CC0PiAnl/microtrees/mar16/job_NeutAirAllSystV5_out/allMerged.root");  
  //NuWro MC NuWro DATA
    inpRun->SetupFromHighland("/data/t2k/dolan/xsToolBasedMEC/CC0PiAnl/microtrees/mar16/job_NuwroAllSystV1_out/allMerged.root","/data/t2k/dolan/xsToolBasedMEC/CC0PiAnl/microtrees/mar16/job_NuwroAllSystV1_out/allMerged.root");  
    //inpRun->SetPOT(4.011E21,4.011E21,4.011E21); // From files
    //inpRun->SetPOT(5.297E21,5.297E21,5.297E21);  // From xsec using rooTracker
    //inpRun->SetPOT(3,5,2);  
  ...
  //All sample signal
     inp->SetSignal("(mectopology==1 || mectopology==2) && (truep_truemom>450) && (truep_truecostheta>0.4) && (truemu_mom>250) && (truemu_costheta>-0.6) && (truep_truemom<1000) ");  
  //TPC only signal
    // inp->SetSignal("(mectopology==1 || mectopology==2) && (truep_truemom>500) && (truep_truecostheta>0.6) && (truemu_mom>250) && (truemu_costheta>0.6) && (truep_truemom<1000) ");  


  //SHOULD USE THIS SIGNAL
  //inp->SetSignal("( (truep_truemom>400) && ( (mectopology==1) || (mectopology==2) ) )");  
...
/*
  const int nTbins = 10;
  const double Tbins[nTbins+1] = { 0.01, 0.3, 0.6, 0.9, 1.2, 1.5, 1.8, 2.1, 2.4, 2.7, 3.0 };
  binningTru->addDimension("trueMrec",nTbins,Tbins);

  const int nRbins = 10;
  const double Rbins[nRbins+1] = {  0.01, 0.3, 0.6, 0.9, 1.2, 1.5, 1.8, 2.1, 2.4, 2.7, 3.0 };
  binningRec->addDimension(string("Sum$(recMrec[xstool_throw])"),nRbins,Rbins);
*/
