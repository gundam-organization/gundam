...
NuclearParameters::~NuclearParameters() { ; }

void NuclearParameters::InitEventMap(std::vector<AnaSample*>& sample, int mode)
{
    InitParameters();
    if(Npar == 0)
    {
        std::cerr << ERR << "In NuclearParameters::InitEventMap\n"
    ...
    for(std::size_t s = 0; s < sample.size(); ++s)
    {
        ...
        for(int i = 0; i < sample[s] -> GetN(); i++)
        {
            ...
            std::vector<NuclearDial> &v_dials = m_dials.at(sample[s] -> GetDetector());
...
void NuclearParameters::InitParameters()
{
...
void NuclearParameters::ReWeight(AnaEvent* event, const std::string& det, int nsample, int nevent, std::vector<double>& params)
{
    if(m_dial_evtmap.empty()) // need to build an event map first
    {
        std::cerr << ERR << "In NuclearParameters::ReWeight()\n"
                  << ERR << "Need to build event map index for " << m_name << std::endl;
        return;
    }

    std::vector<NuclearDial> &v_dials = m_dials.at(det);
    int num_dials = v_dials.size();
    double weight = 1.0;
...
void NuclearParameters::AddDetector(const std::string& det, const std::string& config)
{
    ...
    std::vector<NuclearDial> v_dials;
    for(const auto& dial : j["dials"])
    {
        if(dial["use"] == true)
        {
            ...
            NuclearDial x(dial["name"], fname_binning, fname_splines);
