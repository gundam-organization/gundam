...
#define INCLUDE_DEPRECATED_DIAL_TYPES
    ...
    LogAlertOnce << "DEPRECATED DIAL-TYPE USED: MonotonicSpline will be removed"
            << std::endl;
    SplineDialBaseFactory factory;
    ...
    LogAlertOnce << "DEPRECATED DIAL-TYPE USED: GeneralSpline will be removed"
            << std::endl;
    SplineDialBaseFactory factory;
    dialBase.reset(factory("Spline", "not-a-knot", dialInitializer,cached));
  }
  else if (dialType == "SimpleSpline") {
    LogAlertOnce << "DEPRECATED DIAL-TYPE USED: SimpleSpline will be removed"
            << std::endl;
    SplineDialBaseFactory factory;
    dialBase.reset(factory("Spline", "knot-a-knot", dialInitializer,cached));
  }
  else if (dialType == "LightGraph") {
    LogAlertOnce << "DEPRECATED DIAL-TYPE USED: LightGraph will be removed"
            << std::endl;
    GraphDialBaseFactory factory;
