  ...
  if( _monitor_.isEnabled ){
    ...
    if( _monitor_.gradientDescentMonitor.isEnabled ){
      ...
      if( isGradientDescentStep ){
        ...
        LogWarning << _owner_->getLikelihoodInterface().getBuffer().totalLikelihood << std::endl;
        gradient.stepPointList.back().parState = _owner_->getPropagator().getParametersManager().exportParameterInjectorConfig();
        gradient.stepPointList.back().llh = _owner_->getLikelihoodInterface().getBuffer().totalLikelihood;
...
//    t << (double)_monitor_.itSpeed.counts / (double)_monitor_.itSpeed.cumulated * 1E6 << " it/s" << GenericToolbox::TablePrinter::NextColumn;
      ...
      _monitor_.convergenceMonitor.getVariable("Total/dof").addQuantity(
          _owner_->getLikelihoodInterface().getBuffer().totalLikelihood / _owner_->getLikelihoodInterface().getNbDof()
      );
      _monitor_.convergenceMonitor.getVariable("Total").addQuantity( _owner_->getLikelihoodInterface().getBuffer().totalLikelihood );
      _monitor_.convergenceMonitor.getVariable("Stat").addQuantity( _owner_->getLikelihoodInterface().getBuffer().statLikelihood );
      _monitor_.convergenceMonitor.getVariable("Syst").addQuantity( _owner_->getLikelihoodInterface().getBuffer().penaltyLikelihood );
  ...
  if( _throwOnBadLlh_ and not _owner_->getLikelihoodInterface().getBuffer().isValid() ){
    LogError << _owner_->getLikelihoodInterface().getSummary() << std::endl;
