...
namespace JointProbability{
  ...
  double BarlowBeeston::eval(const SamplePair& samplePair_, int bin_) const {
    _buf_.rel_var = samplePair_.model->getHistogram().binList[bin_].error / TMath::Sq(samplePair_.model->getHistogram().binList[bin_].content);
    _buf_.b       = (samplePair_.model->getHistogram().binList[bin_].content * _buf_.rel_var) - 1;
    _buf_.c       = 4 * samplePair_.data->getHistogram().binList[bin_].content * _buf_.rel_var;

    _buf_.beta   = (-_buf_.b + std::sqrt(_buf_.b * _buf_.b + _buf_.c)) / 2.0;
    _buf_.mc_hat = samplePair_.model->getHistogram().binList[bin_].content * _buf_.beta;

    // Calculate the following LLH:
    //-2lnL = 2 * beta*mc - data + data * ln(data / (beta*mc)) + (beta-1)^2 / sigma^2
    // where sigma^2 is the same as above.
    _buf_.chi2 = 0.0;
    if(samplePair_.data->getHistogram().binList[bin_].content <= 0.0) {
      _buf_.chi2 = 2 * _buf_.mc_hat;
      _buf_.chi2 += (_buf_.beta - 1) * (_buf_.beta - 1) / _buf_.rel_var;
    }
    else{
      _buf_.chi2 = 2 * (_buf_.mc_hat - samplePair_.data->getHistogram().binList[bin_].content);
      if(samplePair_.data->getHistogram().binList[bin_].content > 0.0) {
        _buf_.chi2 += 2 * samplePair_.data->getHistogram().binList[bin_].content *
                      std::log(samplePair_.data->getHistogram().binList[bin_].content / _buf_.mc_hat);
      }
      _buf_.chi2 += (_buf_.beta - 1) * (_buf_.beta - 1) / _buf_.rel_var;
    }
    return _buf_.chi2;
