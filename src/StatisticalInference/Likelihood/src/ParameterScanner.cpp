...
void ParameterScanner::scanParameter(Parameter& par_, TDirectory* saveDir_) {
  ...
  if( par_.getOwner()->isEnableEigenDecomp() and not par_.isEigen() ){
    _likelihoodInterfacePtr_->getDataSetManager().getModelPropagator().setEnableEigenToOrigInPropagate( true );
  }
...
void ParameterScanner::scanSegment(TDirectory *saveDir_, const JsonType &end_, const JsonType &start_, int nSteps_) {
  ...
  auto currentParState = _likelihoodInterfacePtr_->getDataSetManager().getModelPropagator().getParametersManager().exportParameterInjectorConfig();

  LogInfo << "Reading start point parameter state..." << std::endl;
  std::vector<std::pair<Parameter*, double>> startPointParValList;
  if( not start_.empty() ){ _likelihoodInterfacePtr_->getDataSetManager().getModelPropagator().getParametersManager().injectParameterValues(start_); }
  else{ _likelihoodInterfacePtr_->getDataSetManager().getModelPropagator().getParametersManager().injectParameterValues(currentParState); }
  for( auto& parSet : _likelihoodInterfacePtr_->getDataSetManager().getModelPropagator().getParametersManager().getParameterSetsList() ){
  ...
  _likelihoodInterfacePtr_->getDataSetManager().getModelPropagator().getParametersManager().injectParameterValues(end_);
  ...
  _likelihoodInterfacePtr_->getDataSetManager().getModelPropagator().getParametersManager().injectParameterValues( currentParState );
  _likelihoodInterfacePtr_->propagateAndEvalLikelihood();
}
void ParameterScanner::generateOneSigmaPlots(TDirectory* saveDir_){
  ...
  _likelihoodInterfacePtr_->getDataSetManager().getModelPropagator().propagateParameters();
  _likelihoodInterfacePtr_->getDataSetManager().getModelPropagator().getPlotGenerator().generateSamplePlots();
  auto refHistList = _likelihoodInterfacePtr_->getDataSetManager().getModelPropagator().getPlotGenerator().getHistHolderList();
  ...
  auto makeOneSigmaPlotFct = [&](Parameter& par_, TDirectory* parSavePath_){
    ...
    _likelihoodInterfacePtr_->getDataSetManager().getModelPropagator().getPlotGenerator().generateSampleHistograms(nullptr, 1);

    // Compare with the
    _likelihoodInterfacePtr_->getDataSetManager().getModelPropagator().getPlotGenerator().generateComparisonPlots(
        _likelihoodInterfacePtr_->getDataSetManager().getModelPropagator().getPlotGenerator().getHistHolderList(1),
...
void ParameterScanner::varyEvenRates(const std::vector<double>& paramVariationList_, TDirectory* saveDir_){
  ...
  auto makeVariedEventRatesFct = [&](Parameter& par_, std::vector<double> variationList_, TDirectory* saveSubDir_){
    ...
    for( size_t iSample = 0 ; iSample < _likelihoodInterfacePtr_->getDataSetManager().getModelPropagator().getSampleSet().getSampleList().size() ; iSample++ ){
      ...
      GenericToolbox::writeInTFile(
          ...
          _likelihoodInterfacePtr_->getDataSetManager().getModelPropagator().getSampleSet().getSampleList()[iSample].getName()
      );
