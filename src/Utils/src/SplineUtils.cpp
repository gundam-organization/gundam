...
namespace SplineUtils{
  ...
  std::vector<SplinePoint> getSplinePointList(const TSpline3* src_){
    ...
    for( int iPt = 0 ; iPt < src_->GetNp(); iPt ++ ){
      ...
      if( src_->GetNp() == 1 ) { point.slope = 0; continue; }
  ...
  void fillCatmullRomSlopes(std::vector<SplinePoint>& splinePointList_){
    ...
    if( nPoints == 1 ){ splinePointList_[0].slope = 0; return; }

    // From here we have at least 2 points

    // extremity points
    size_t idx{0};
    idx = 0;         splinePointList_[idx].slope = getSlope(splinePointList_[idx], splinePointList_[idx+1]);
    idx = nPoints-1; splinePointList_[idx].slope = getSlope(splinePointList_[idx-1], splinePointList_[idx]);
  ...
  void fillAkimaSlopes(std::vector<SplinePoint>& splinePointList_){
    ...
    // Catmull-rom style
    idx = 1; splinePointList_[idx].slope = getSlope(splinePointList_[idx-1], splinePointList_[idx+1]);
    if( nPoints == 3 ){ return; } // stop there

    idx = nPoints-2; splinePointList_[idx].slope = getSlope(splinePointList_[idx-1], splinePointList_[idx+1]);
    ...
    for( size_t iPt = 2; iPt < nPoints-2 ; iPt++ ){
      ...
      if( n0 > 1E-6 or n1 < 1E-6 ){ splinePointList_[iPt].slope = ( n1*mm1 + n0*m0 )/( n1 + n0 ); }
      else                        { splinePointList_[iPt].slope = 0.5*( m0 + mm1 ); }
    }
  ...
  bool isUniform(const std::vector<SplinePoint>& splinePointList_, double tolerance_){
    ...
    for( size_t i=0; i< splinePointList_.size()-1; ++i) {
      ...
      const double delta = std::abs(splinePointList_[i].x - splinePointList_[0].x - static_cast<double>(i)*avgSpace)/avgSpace;
