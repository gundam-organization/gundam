...
public:
  virtual ~CalcLLHFunc() {};
  virtual double operator()(double mc, double w2, double data, double data_w2 = 1)
  {
    return 0.0;
  }
};
...
class BarlowLLHFixWeight : public CalcLLHFunc {
public:
  double rel_var, b, c, beta, mc_hat, chi2;

  double operator()(double mc_, double w2_, double data_, double data_w2_) {
    if(mc_ == data_ ) return 0;
    // Solving for the quadratic equation,
    // beta^2 + (mu * sigma^2 - 1)beta - data * sigma^2) = 0
    // where sigma^2 is the relative variance.
    rel_var = data_w2_ / (mc_ * mc_);
    b       = (mc_ * rel_var) - 1;
    c       = 4 * data_ * rel_var;
