...
void EventTreeWriter::writeEvents(TDirectory* saveDir_, const std::string& treeName_, int sampleIdx_, const EventDialCache & eventDialCache_) const{
  LogThrowIf(saveDir_ == nullptr, "Save TDirectory is not set.");
  LogThrowIf(treeName_.empty(), "TTree name no set.");

  LogReturnIf(eventDialCache_.getCache().empty(), "No event to be writen. Leaving...");
  LogInfo << "Writing " << eventDialCache_.getCache().size() << " events in TTree " << treeName_ << std::endl;

  eventDialCache_.getCache()[0].event->getSampleIndex();
  ...
  for( auto& leafDef : leafDictionary ){
    if( not leavesDefStr.empty() ) leavesDefStr += ":";
    leavesDefStr += leafDef.first;
    leafDef.second(privateMemberArr, *eventDialCache_.getCache()[0].event); // resize buffer
  }
  privateMemberArr.lockArraySize();
  tree->Branch("Event", &privateMemberArr.getRawDataArray()[0], leavesDefStr.c_str());

  GenericToolbox::RawDataArray loadedLeavesArr;
  auto loadedLeavesDict = eventDialCache_.getCache()[0].event->generateLeavesDictionary(true);
  std::vector<std::string> leafNamesList;
  if( not loadedLeavesDict.empty() ){
    leavesDefStr = "";
    for( auto& leafDef : loadedLeavesDict ){
      ...
      leavesDefStr += leafDef.first;
      leafNamesList.emplace_back(leafDef.first.substr(0,leafDef.first.find("[")).substr(0, leafDef.first.find("/")));
      leafDef.second(loadedLeavesArr, eventDialCache_.getCache()[0].event->getLeafHolder(leafNamesList.back())); // resize buffer
    }
    loadedLeavesArr.lockArraySize();
    tree->Branch("Leaves", &loadedLeavesArr.getRawDataArray()[0], leavesDefStr.c_str());
  }
...
//          if( std::isnan( min ) ) min = par.getPriorValue() - 5*par.getStdDevValue();
//          if( std::isnan( max ) ) max = par.getPriorValue() + 5*par.getStdDevValue();
...
//  }
  ...
  size_t iEvent{0}; size_t nEvents = (eventDialCache_.getCache().size());
  for( auto& cacheEntry : eventDialCache_.getCache() ){
    GenericToolbox::displayProgressBar(iEvent++,nEvents,progressTitle);

    privateMemberArr.resetCurrentByteOffset();
    for( auto& leafDef : leafDictionary ){ leafDef.second(privateMemberArr, *cacheEntry.event); }

    iLeaf = 0;
    loadedLeavesArr.resetCurrentByteOffset();
    for( auto& leafDef : loadedLeavesDict ){ leafDef.second(loadedLeavesArr, cacheEntry.event->getLeafHolder(leafNamesList[iLeaf++])); }
