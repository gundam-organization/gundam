...
// The warning about altering the nominal params implies there's something wrong with your response
// function. You should have+/- 3 sigma relative to the nominal, if you're shifting the nominal make
// sure you're doing this intentionally

// store response functions in vector of FSI "bins" (Ereco, Etrue, reac, topo)
void FSIParameters::StoreResponseFunctions(vector<TFile*> respfuncs,
                    ...
                    { // save memory if reco bin and true bin very far away
                        for(uint i = 0; i < Npar; i++)
                        {
                            ...
                            if(!g)
                            {
                                if(rtInt != 0 && rtInt < 6)
                                { // Ignore OOFV splines
                                    cout << "Warning, creating dummy respfunc, param: " << i << " "
                                         << name << endl;
                            ...
                            if((g->GetY())[3] != 1.0)
                            {
                                cout << "Warning: altering fsi nominal param: " << i << " " << name
                                     << endl;
                            }
...
// --
int FSIParameters::GetBinIndex(SampleTypes sampletype, ReactionTypes reactype, double D1reco,
...
void FSIParameters::InitEventMap(std::vector<AnaSample*>& sample, int mode)
{
    if(m_bins.empty())
    {
        cout << "Need to build map of response functions for " << m_name << " ... exiting ..."
             << endl;
        exit(-1);
    }
    m_evmap.clear();
...
// EventWeghts
void FSIParameters::EventWeights(std::vector<AnaSample*>& sample, std::vector<double>& params)
{
...
// ReWeight
void FSIParameters::ReWeight(AnaEvent* event, int nsample, int nevent, std::vector<double>& params)
{
