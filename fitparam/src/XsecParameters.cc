...
void XsecParameters::StoreResponseFunctions(std::vector<TFile*> respfuncs, std::vector<std::pair <double,double> > v_D1edges,
                ...
                for(int bt=0;bt<nccqebins;bt++){//true kinematics bin
                    ...
                    if(fabs(br-bt)<21) {  //save memory if reco bin and true bin very far away
                        for(uint i=0; i<Npar; i++){
                            ...
                            //cout<<g<<endl;
                            if(!g){
                                if(rtInt!=0 && rtInt<6 && i<9){ // Ignore OOFV splines, don't bother with warnings for lack of CCQE params
                                    std::cout << "WARNING: creating dummy respfunc, param: " << i << " " << name << std::endl;
                            ...
                            g->SetName(name);
                            if((g->GetY())[3]!=1.0){
                                std::cout << "WARNING: altering xsec nominal param: " << i << " " << name << std::endl;
                            }
...
void XsecParameters::InitEventMap(std::vector<AnaSample*> &sample, int mode)
{
    if(m_bins.empty())
    {
        std::cout<<"Need to build map of response functions for "<<m_name<<" ... exiting ..."<<std::endl;
    }
    m_evmap.clear();
    ...
    for(size_t s=0;s<sample.size();s++)
    {
        ...
        for(int i=0;i<sample[s]->GetN();i++)
        {
            ...
            if(binn == BADBIN)
            {
                ...
                std::cout<<" This event will be ignored in analysis."
                    <<std::endl;
                ev->Print();
            }
