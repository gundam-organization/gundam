#!/bin/bash
# Wrap a ROOT macro as a script.
#
#  Check the output of GUNDAM 200HorrifyingMCMCCov.sh against the
#  expected values.
#
root -b -n <<EOF
#include <iostream>
#include <string>
#include <memory>
#include <cmath>

#include <TFile.h>
#include <TH1F.h>
#include <TTree.h>

std::string args{"$*"};
int status{0};

/// Fail with message if "v1" evaluates to false.  THIS IS COPIED
/// HERE TO AVOID DEPENDENCIES
#define EXPECT(msg,v1)                                      \
    do {                                                    \
        if (not (v1)) {                                     \
            std::cout << "FAIL:";                           \
            ++ status;                                      \
        } else {                                            \
            std::cout << "SUCCESS:";                        \
        }                                                   \
        std::cout << " " << msg                             \
                  << " [ (" << #v1 << ") --> " << v1 << "]" \
                  << std::endl;                             \
    } while (false)

/// Fail if fractional difference between "v1" and "v2" is larger than "tol"
/// THIS IS COPIED HERE TO AVOID DEPENDENCIES
#define TOLERANCE(msg,v1,v2,tol)                            \
    do {                                                    \
        double v = (v1)>0 ? (v1): -(v1);                    \
        double vv = (v2)>0 ? (v2): -(v2);                   \
        double d = std::abs((v1)-(v2));                     \
        double r = d/std::max(0.5*(v+vv),(tol));            \
        if (r > (tol)) {                                    \
            std::cout << "FAIL:";                           \
            ++ status;                                      \
        } else {                                            \
            std::cout << "SUCCESS:";                        \
        }                                                   \
        std::cout << " " << msg                             \
                  << std::setprecision(8)                   \
                  << std::scientific                        \
                  << " (" << r << "<" << (tol) << ")"       \
                  << " [" << #v1 << "=" << (v1)             \
                  << " " << #v2 << "=" << (v2)              \
                  << " " << d << "]"                        \
                  << std::endl;                             \
    } while(false);

int main() {
    std::shared_ptr<TFile> mcmc(new TFile("200HorrifyingMCMCCov.root","old"));

    TTree* tree
        = dynamic_cast<TTree*>(mcmc->Get(
                                 "FitterEngine"
                                 "/fit"
                                 "/MCMC"));
    std::vector<double> points;
    std::vector<double>* addrPoints = &points;
    tree->SetBranchAddress("Points",&addrPoints);

    TH1F normA("normA", "The first parameter",100, 0.0, 2.0);
    TH1F normB("normB", "The second parameter",100, 0.0, 2.0);

    for (int i=0; i < tree->GetEntries(); ++i) {
        tree->GetEntry(i);
        normA.Fill(points[0]);
        normB.Fill(points[1]);
    }

    normA.Draw();
    gPad->Print("900HorrifyingMCMCCovCheck.pdf(");
    normB.Draw();
    gPad->Print("900HorrifyingMCMCCovCheck.pdf)");

    // The expected values are for the data generated by
    // 100NormalizationTree.C.  They need to be changed if that tree is
    // changed.
    TOLERANCE("Check MCMC matches expectation value for #0_Positive_C",
              normA.GetMean(),
              1.0, 5E-2);
    TOLERANCE("Check MCMC RMS matches expectation value for #0_Positive_C",
              normA.GetRMS(),
              0.21213, 5E-2);
    TOLERANCE("Check MCMC matches expectation value for #1_Negative_C",
              normB.GetMean(),
              1.0, 5E-2);
    TOLERANCE("Check MCMC RMS matches expectation value for #1_Negative_C",
              normB.GetRMS(),
              0.21213, 5E-2);

    mcmc->Close();

    return status;
}
exit(main());
EOF
# Local Variables:
# mode:c++
# c-basic-offset:4
# End:
