...
#
# Check MinimizerBase::evalFitGradient on spline dials against a centered
# finite difference.
...
bool isSplineParameter(const Parameter* par) {
    if( par == nullptr ){ return false; }
    return par->getName() == "spline_C" or par->getName() == "spline_D";
}

int main() {
    std::string configFile{args + "/200CovarianceFit-config.yaml"};
    ...
    std::shared_ptr<TFile> outputFile(new TFile("902EvalFitGradientSplineCheck.root", "recreate"));
    ...
    EXPECT("Four covariance fit parameters are expected", fitParameters.size() == 4);
    if( fitParameters.size() != 4 ){ return status; }

    std::vector<std::size_t> splineParameterIndices;
    std::vector<Parameter*> splineParameters;
    for( std::size_t iPar = 0 ; iPar < fitParameters.size() ; iPar++ ){
        if( not isSplineParameter(fitParameters[iPar]) ){ continue; }
        splineParameterIndices.emplace_back(iPar);
        splineParameters.emplace_back(fitParameters[iPar]);
    }

    EXPECT("Two spline fit parameters are expected", splineParameters.size() == 2);
    if( splineParameters.size() != 2 ){ return status; }
    ...
    minimizer.evalFitGradient(splineParameters);
    ...
    gradientValues.reserve(splineParameters.size());
    for( auto* par : splineParameters ){
        gradientValues.emplace_back(par->getGradient());
    }

    const double step{1E-4};
    const double tolerance{1E-2};
    for( std::size_t iSpline = 0 ; iSpline < splineParameters.size() ; iSpline++ ){
        std::vector<double> shiftedValues{nominalValues};
        const std::size_t iPar{splineParameterIndices[iSpline]};
        ...
        TOLERANCE(
            std::string("evalFitGradient matches finite difference for spline parameter ")
                + splineParameters[iSpline]->getFullTitle(),
            gradientValues[iSpline], finiteDiff, tolerance
        );
    }
