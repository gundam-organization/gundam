                      ...
                      m_binning(binning), m_data_tree(t_data), m_norm(1.0)
{
    ...
    {
        std::cout << bin.D2low << " " << bin.D2high << " "
                  << bin.D1low << " " << bin.D1high << std::endl;
    }
...
void AnaSample::AddEvent(const AnaEvent& event)
{
...
void AnaSample::GetSampleBreakdown(TDirectory *dirout, const std::string& tag, const std::vector<std::string>& topology, bool save)
{
    ...
    for(std::size_t i = 0; i < m_events.size(); ++i)
    {
        double D1_rec, D2_rec, D1_true, D2_true, wght;
