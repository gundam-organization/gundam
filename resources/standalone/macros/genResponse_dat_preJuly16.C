...
void genResponse_dat(string microtreeIn, string rwpath, string dialName, double nom, double err, int topo, int reac, int dataType=0) {
  ...
  const int nRbins = 8;
  const double Rbins[nRbins+1] = { 0.0, 0.47, 1.02, 1.54, 1.98, 2.34, 2.64, 2.89, 3.14159};


  // set up true binning
  const int nTbins = 8;
  const double Tbins[nTbins+1] = { 0.0, 0.47, 1.02, 1.54, 1.98, 2.34, 2.64, 2.89, 3.14159};
  ...
  for(const int bt = 0; bt < nTbins; bt++){
    for(const int bpm = 0; bpm < npmombins; bpm++){
      for(const int bpt = 0; bpt < npthetabins; bpt++){
        ...
        cout << "Current bins (global DalphaT protonMom protonAngle): " << globalCount << " " << bt << " " << bpm << " " << bpt << endl << endl;
        for(const int w = 0; w < nWeights; w++){
          ...
          signal = Form("(trueDalphaT) > %f && (trueDalphaT) < %f  && (truep_truemom) > %f && (truep_truemom) < %f && (truep_truecostheta) > %f && (truep_truecostheta) < %f", Tbins[bt],Tbins[bt+1],pmombins[bpm],pmombins[bpm+1],pthetabins[bpt],pthetabins[bpt+1]);
          bcut = Form("((Sum$(trueDalphaT[xstool_throw]) > %f) && (Sum$(trueDalphaT[xstool_throw])) < %f) && ((Sum$(truep_truemom[xstool_throw]) > %f) && (Sum$(truep_truemom[xstool_throw])) < %f) && ((Sum$(truep_truecostheta[xstool_throw]) > %f) && (Sum$(truep_truecostheta[xstool_throw])) < %f)", Tbins[bt],Tbins[bt+1],pmombins[bpm],pmombins[bpm+1],pthetabins[bpt],pthetabins[bpt+1]);
