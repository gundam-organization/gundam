...
int treeConvert(TString inFileName, TString inTreeName,  TString inTreeName_T, TString outFileName,
                ...
                TString D2NameRec, TString D2NameTrue, TString D2NameTrue_T, bool isRealData=false)
{
  ...
  const int accumToCut[nbranches] =   {8,8,10,9,8,5,4,8,9,8};
    ...
    for(int i=0; i<nbranches; i++){
      if(accum_level[0][i]>accumToCut[i]){
        ...
        if(psdim){
          ...
          const double pmombins[npmombins+1] = {0.0, 500.0, 1000.0, 100000.0};
          const int npthetabins=2;
          const double pthetabins[npthetabins+1] = {-1.0, 0.6, 1.0};
