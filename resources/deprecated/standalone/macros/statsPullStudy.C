...
void statsPullStudy(bool fitsyst=true, const char * outFileName="statsPullStudyOut.root", const int nbins=9, const char *dirname="./", const char *ext=".root")
{
  ...
  Float_t params_flux[1000][nbins];
  Float_t paramsErrs_flux[1000][nbins];
  Float_t paramsTruth_flux[1000][nbins];
  Float_t params_det[1000][nbins];
  Float_t paramsErrs_det[1000][nbins];
  Float_t paramsTruth_det[1000][nbins];
  Float_t params_xsec[1000][nbins];
  Float_t paramsErrs_xsec[1000][nbins];
  Float_t paramsTruth_xsec[1000][nbins];
  Float_t params_PionFSI[1000][nbins];
  Float_t paramsErrs_PionFSI[1000][nbins];
  Float_t paramsTruth_PionFSI[1000][nbins];
  int fileCount=0;
  if(files){
    TSystemFile *file;
    TString fname;
    ...
    while ((file=(TSystemFile*)next())) {
      ...
      if(!file->IsDirectory() && fname.EndsWith(ext) && fname!=outFileName) {
        ...
        if(!paramVals || !paramErrs) continue;
        Int_t nccqepar = paramVals->GetEntries();
        ...
        if(fitsyst){
          ...
          if(!paramVals_flux || !paramErrs_flux) continue;
          Int_t nfluxpar = paramVals_flux->GetEntries();
          ...
          if(!paramVals_det || !paramErrs_det) continue;
          Int_t ndetpar = paramVals_det->GetEntries();
          ...
          if(!paramVals_xsec || !paramErrs_xsec) continue;
          Int_t nxsecpar = paramVals_xsec->GetEntries();
          ...
          if(!paramVals_PionFSI || !paramErrs_PionFSI) continue;
          Int_t npionfsipar = paramVals_PionFSI->GetEntries();
          ...
          for(int i=0;i<nfluxpar;i++){
            ...
            paramsTruth_flux[fileCount][i]=1.0;
          }
          for(int i=0;i<ndetpar;i++){
            ...
            paramsTruth_det[fileCount][i]=1.0;
          }
          for(int i=0;i<nxsecpar;i++){
            ...
            paramsTruth_xsec[fileCount][i]=1.0;
          }
          for(int i=0;i<npionfsipar;i++){
            ...
            paramsTruth_PionFSI[fileCount][i]=1.0;
          }
        }
          inFile->Close();
        fileCount++;
  ...
  Float_t cumean[100] = {};
  int incCount[100] = {};
  tree->Branch("nbins", &nbins, "nbins/I");
  ...
  tree->Branch("fitParams_flux", fitParams_flux, "fitParams_flux[nbins]/F");
  tree->Branch("fitParamsErrs_flux", fitParamsErrs_flux, "fitParamsErr_fluxs[nbins]/F");
  tree->Branch("TruthParams_flux", TruthParams_flux, "TruthParams_flux[nbins]/F");
  tree->Branch("fitParams_det", fitParams_det, "fitParams_det[nbins]/F");
  tree->Branch("fitParamsErrs_det", fitParamsErrs_det, "fitParamsErrs_det[nbins]/F");
  tree->Branch("TruthParams_det", TruthParams_det, "TruthParams_det[nbins]/F");
  tree->Branch("fitParams_xsec", fitParams_xsec, "fitParams_xsec[nbins]/F");
  tree->Branch("fitParamsErrs_xsec", fitParamsErrs_xsec, "fitParamsErrs_xsec[nbins]/F");
  tree->Branch("TruthParams_xsec", TruthParams_xsec, "TruthParams_xsec[nbins]/F");
  tree->Branch("fitParams_pionfsi", fitParams_pionfsi, "fitParams_pionfsi[nbins]/F");
  tree->Branch("fitParamsErrs_pionfsi", fitParamsErrs_pionfsi, "fitParamsErrs_pionfsi[nbins]/F");
  tree->Branch("TruthParams_pionfsi", TruthParams_pionfsi, "TruthParams_pionfsi[nbins]/F");
  tree->Branch("Pull_ccqe", Pull_ccqe, "Pull_ccqe[nbins]/F");
  tree->Branch("Pull_flux", Pull_flux, "Pull_flux[nbins]/F");
  tree->Branch("Pull_det", Pull_det, "Pull_det[nbins]/F");
  tree->Branch("Pull_xsec", Pull_xsec, "Pull_xsec[nbins]/F");
  tree->Branch("Pull_pionfsi", Pull_pionfsi, "Pull_pionfsi[nbins]/F");
  for(int i=0;i<fileCount;i++){
    cout << "On file: " << i << endl;
    mean0=params[i][0];
    mean1=params[i][1];
    for(int j=0;j<nccqepar;j++){
      cout << i << "  " << j << "   " << params[i][j] << endl;
      if(params[i][j]<10 && params[i][j]>0){
        fitParams[j]=params[i][j];
        fitParamsErrs[j]=paramsErrs[i][j];
        TruthParams[j]=paramsTruth[i][j];
        if(fitParamsErrs[j]>0)Pull_ccqe[j]=(fitParams[j]-TruthParams[j])/fitParamsErrs[j];
    ...
    for(int j=0;j<nfluxpar;j++){
      ...
      if(fitParamsErrs_flux[j]>0)Pull_flux[j]=(fitParams_flux[j]-TruthParams_flux[j])/fitParamsErrs_flux[j];;
    }
    for(int j=0;j<ndetpar;j++){
      ...
      if(fitParamsErrs_det[j]>0)Pull_det[j]=(fitParams_det[j]-TruthParams_det[j])/fitParamsErrs_det[j];;
    }
    for(int j=0;j<nxsecpar;j++){
      ...
      if(fitParamsErrs_xsec[j]>0)Pull_xsec[j]=(fitParams_xsec[j]-TruthParams_xsec[j])/fitParamsErrs_xsec[j];;
    }
    for(int j=0;j<npionfsipar;j++){
      ...
      if(fitParamsErrs_pionfsi[j]>0)Pull_pionfsi[j]=(fitParams_pionfsi[j]-TruthParams_pionfsi[j])/fitParamsErrs_pionfsi[j];;
    }    
    tree->Fill();
  }
